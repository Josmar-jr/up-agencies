      ...
      components={{
        ...
        Dropdown: (props) => {
          ...
          if (props.name === 'months') {
            const mothsArray = Array.from({
              length: 12,
            }).map((_, i) => {
              return {
                label: dayjs(new Date()).month(i).format('MMM'),
                value: i.toString(),
              }
            })
            ...
            return (
              <Select
                onValueChange={(newValue) => {
                  const newDate = dayjs(new Date(currentMonth)).set(
                    'month',
                    Number(newValue)
                  )

                  goToMonth(newDate.toDate())
                }}
          ...
          if (props.name === 'years') {
            ...
            let yearsArray: { label: string; value: string }[] = []
